...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'bookings') THEN
    EXECUTE 'UPDATE public.bookings b SET "tenantId" = COALESCE(u."tenantId", sr."tenantId", s."tenantId") FROM public.users u LEFT JOIN public."ServiceRequest" sr ON sr.id = b."serviceRequestId" LEFT JOIN public.services s ON s.id = b."serviceId" WHERE b."clientId" = u.id AND b."tenantId" IS NULL';
  END IF;
END$$;
