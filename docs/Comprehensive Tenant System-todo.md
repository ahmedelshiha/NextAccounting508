...
**Subtasks:**
- [x] Convert high-risk admin routes (services, service-requests, tasks) to withTenantContext
- [x] Convert portal realtime, chat, booking-preferences and service-requests to withTenantContext
- [ ] Convert remaining server routes using getServerSession (bookings, payments, register, email/test)

**Examples of converted files:**
- src/app/api/admin/service-requests/** (GET/POST, subroutes) — uses withTenantContext + requireTenantContext
- src/app/api/portal/chat/route.ts — uses withTenantContext and requireTenantContext (see code for details)
- src/app/api/users/me/route.ts — refactored to withTenantContext

**AI Agent Steps:**
```bash
# Step 1: Find remaining routes using getServerSession
rg "getServerSession" src/app/api -n > remaining_getServerSession.txt
cat remaining_getServerSession.txt

# Step 2: For each file, refactor export handlers to use withTenantContext
# (Manual step: verify logic uses tenantContext and requireTenantContext)

# Validation: run eslint custom rule to detect direct getServerSession usage
