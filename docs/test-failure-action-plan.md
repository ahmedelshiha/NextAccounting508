...
- **Actions Taken**:
  ...
  - Updated key admin endpoints to use the centralized response helper `respond.forbidden('Forbidden')` for consistency.
- **Files Modified**:
  ...
  - `src/app/api/admin/team-management/route.ts`
  - `src/app/api/admin/analytics/route.ts`
- **Notes**: These changes ensure permission failures return 403, avoiding confusion with unauthenticated 401 responses generated by `withTenantContext`. Recommended next steps: run targeted tests for these routes and complete a full audit across all remaining admin endpoints to apply the same pattern.
