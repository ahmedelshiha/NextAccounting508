...
| F7 | Env/tooling references drift | `package.json` scripts, `docs/env-reference.md`, `doppler.yaml` | Medium | Resolved | Updated docs to reflect current environment strategy (platform envs & local `.env.local` with validation); Doppler usage marked as optional in docs; `doppler.yaml` retained as an optional helper. |
