...
- [ ] Wire adapters into hooks to consume /api/admin/tasks
...
- 2025-09-13: Implemented API⇄UI adapters at `temp/task management/lib/tasks/adapters.ts` with priority/status mapping and payload transformers; scaffolded dev admin pages under `temp/task management/src/app/admin/tasks/` (page.tsx, [id]/page.tsx, loading.tsx). Next: wire adapters into a dev hook to fetch `/api/admin/tasks` and hydrate UI Task[].
