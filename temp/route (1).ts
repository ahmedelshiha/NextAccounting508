...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build where clause
    const where: { status?: string; OR?: ({ clientName?: { contains: string; mode: 'insensitive' }; clientEmail?: { contains: string; mode: 'insensitive' }; service?: { name?: { contains: string; mode: 'insensitive' } } })[]; scheduledAt?: { gte?: Date; lte?: Date } } = {}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const bookingData: { serviceId: string; scheduledAt: Date; duration: number; notes?: string; status: string; clientId?: string; clientName?: string; clientEmail?: string; clientPhone?: string } = {
      serviceId,
