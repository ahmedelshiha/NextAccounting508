...
describe('admin/client-settings API', () => {
  it('GET returns defaults when missing', async () => {
    const mod = await import('@/app/api/admin/client-settings/route')
    const res: any = await mod.GET(new Request(`${base}/api/admin/client-settings`))
  ...
  it('PUT rejects invalid payload (400)', async () => {
    const mod = await import('@/app/api/admin/client-settings/route')
    const payload = { registration: { requireAccount: 'not-a-bool' } }
    const res: any = await mod.PUT(new Request(`${base}/api/admin/client-settings`, { method: 'PUT', body: JSON.stringify(payload) }))
  ...
  it('PUT accepts valid update and persists (200)', async () => {
    const mod = await import('@/app/api/admin/client-settings/route')
    const payload = { registration: { requireAccount: true, emailVerification: false } }
    const res: any = await mod.PUT(new Request(`${base}/api/admin/client-settings`, { method: 'PUT', body: JSON.stringify(payload) }))
    expect(res.status).toBe(200)
    const out = await res.json()
    expect(out.registration).toBeDefined()
    expect(out.registration.requireAccount).toBe(true)
  })
})
