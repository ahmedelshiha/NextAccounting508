...
interface PendingState { [section: string]: any }
...
export default function CommunicationTab() {
  ...
  function onChange(section: string, field: string, value: any) {
    setPending((prev) => ({ ...prev, [section]: { ...(prev[section]||{}), [field]: value } }))
  }
    ...
    <div className="mt-4">
      <div className="flex items-center justify-between">
        <div className="flex gap-2 text-sm">
          {tabs.map(t => (
            <button key={t.key} onClick={() => setActive(t.key)} className={`px-3 py-1.5 rounded-md border ${active===t.key? 'bg-blue-50 text-blue-700 border-blue-200':'bg-white text-gray-700 border-gray-200 hover:bg-gray-50'}`}>{t.label}</button>
          ))}
        </div>
