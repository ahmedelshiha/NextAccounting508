...
export default function FilterBar({ filters, onFilterChange, onSearch, active = [], searchPlaceholder = '' }: { filters: FilterConfig[]; onFilterChange: (key: string, value: string) => void; onSearch?: (value: string) => void; active?: Array<{ key: string; label: string; value: string }>; searchPlaceholder?: string }) {
  ...
  const ph = searchPlaceholder || t('dashboard.searchPlaceholder.generic')
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6">
      <div className="flex items-center justify-between gap-4">
        {onSearch && (
          <div className="relative flex-1 max-w-md">
            ...
            <input aria-label={ph} onChange={(e) => onSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 text-sm" placeholder={ph} />
          </div>
        )}
      ...
      {active.length > 0 && (
        <div className="mt-3 flex items-center gap-2 flex-wrap">
          <span className="text-sm text-gray-500">{t('dashboard.activeFilters')}</span>
          {active.map((tkn) => (
            <span key={tkn.key} className="flex items-center gap-1 bg-green-50 text-green-700 px-2 py-1 rounded-md text-sm">
              <span>{tkn.label}: {tkn.value}</span>
              <button onClick={() => onFilterChange(tkn.key, '')} className="hover:bg-green-100 rounded p-0.5"><X className="w-3 h-3" /></button>
            </span>
          ))}
        </div>
      )}
