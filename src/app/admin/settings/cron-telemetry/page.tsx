...
import { AlertCircle, Activity, Settings, BarChart3, Shield, Clock } from 'lucide-react'
...
function CronTelemetryContent() {
  ...
  const [aggregatedTenants, setAggregatedTenants] = useState<Record<string, any>>({})
  ...
  async function loadTelemetryData() {
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        setRuns(data.runs || [])
        setAggregatedTenants(data.aggregatedTenants || {})
      }
    } catch (error) {
