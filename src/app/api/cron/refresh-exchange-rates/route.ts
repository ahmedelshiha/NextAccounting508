...
export async function POST(request: NextRequest) {
  try {
    ...
    if (res && (res as any).success) {
      await logAudit({ action: 'exchange:refresh', details: { base, updated: (res as any).updated } })
      return NextResponse.json({ success: true, updated: (res as any).updated })
    }

    await logAudit({ action: 'exchange:refresh:failed', details: { error: (res as any).error ?? 'unknown' } })
