...
export async function POST(request: Request) {
  ...
  try { await logAudit({ action: 'service-request:create', actorId: (session.user as any).id ?? null, targetId: created.id, details: { clientId: created.clientId, serviceId: created.serviceId, priority: created.priority, serviceSnapshot: created.requirements?.serviceSnapshot ?? null } }) } catch {}
