...
export const GET = withTenantContext(async (request: Request) => {
  try {
    ...
    const key = `analytics-settings:export:${tenantId}:${ip}`
    ...
    return NextResponse.json(buildExportBundle('analytics-reporting', settings))
  } catch (e) {
    try { Sentry.captureException(e as any) } catch {}
    return NextResponse.json({ error: 'Failed to export analytics settings' }, { status: 500 })
  }
})
