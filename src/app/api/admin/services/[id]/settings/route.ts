...
export const PATCH = withTenantContext(async (request: NextRequest, context: Ctx) => {
  try {
    ...
    if (!ctx.userId || !hasPermission(role, PERMISSIONS.SERVICES_EDIT)) {
    ...
    if (!settings || typeof settings !== 'object') {
      return NextResponse.json(makeErrorBody({ code: 'INVALID_PAYLOAD', message: 'settings object required' } as any), { status: 400 })
    }

    const result = await svc.bulkUpdateServiceSettings(ctx.tenantId, [{ id, settings }])

    if (result.errors && result.errors.length) {
      return NextResponse.json({ updated: result.updated, errors: result.errors }, { status: 207 })
    }

    await svc.getServiceById(ctx.tenantId, id) // prime cache
