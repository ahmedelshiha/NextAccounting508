...
export const GET = withTenantContext(
  async (req: NextRequest) => {
    ...
    if (provider !== 'netlify') {
      return NextResponse.json({ error: 'Quarantine listing not supported for provider' }, { status: 501 })
    }
...
export const POST = withTenantContext(
  async (req: NextRequest) => {
    const { role, userId } = tenantContext.getContext()
    if (!hasPermission(role, PERMISSIONS.ANALYTICS_VIEW)) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }

    const body = await req.json().catch(() => null)
    if (!body || !body.action || (!body.key && !Array.isArray(body.keys))) {
    ...
    const provider = (process.env.UPLOADS_PROVIDER || '').toLowerCase()
    if (provider !== 'netlify') {
      return NextResponse.json({ error: 'Action not supported for provider' }, { status: 501 })
    }
      ...
      if (action === 'delete') {
        ...
        for (const k of keys) {
          try {
            ...
            try { await logAuditSafe({ action: 'upload:quarantine:delete', actorId: userId ?? null, details: { key: k } }) } catch {}
      ...
      if (action === 'release') {
        ...
        for (const k of keys) {
          try {
            ...
            if (res.ok) {
              try { await logAuditSafe({ action: 'upload:quarantine:release', actorId: userId ?? null, details: { from: k, to: res.key } }) } catch {}
