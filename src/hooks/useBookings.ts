...
export function useBookings(params: BookingsQuery = {}) {
  ...
  // Hint server to return appointments when supported
  query.set('type', 'appointments')
  ...
  const items = scope === 'portal' ? itemsRaw.filter((it: any) => (it?.isBooking === true) || !!(it as any)?.scheduledAt) : itemsRaw
